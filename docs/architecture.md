...
|---|---|---|
| **Fonte** | Onde o dado nasce | `data/raw/` — CSV + JSON "exportados" dos dados do INEP (o que a origem entregou) |
| **Ingestão** | Trazer o dado para o pipeline | `src/ingest.py` (Pandas) |
| **Bronze** | Preservar o que chegou, rastreável e **versionado** | `data/bronze/` — tabelas **Delta Lake** |
| **Silver** | Limpar, tipar, padronizar, integrar | modelos `stg_*` (dbt) |
| **Gold** | Organizar para uma necessidade de consumo | `fato_taxa` + dimensões (dbt) |
...
**2. Compile.** As expressões `{{ }}` (Jinja) são substituídas. O `{{ ref('stg_ufs') }}` que você escreveu vira o endereço real da tabela. O SQL resultante fica em **`target/compiled/`** — é lá que você olha quando quer entender o que o dbt realmente entendeu do seu código.
...
```text
depends_on de dim_uf: ['model.central_servicos.stg_ufs']
```
...
```text
bronze.taxas ──> stg_taxas ──┬──> fato_taxa ──┐
(Delta)                      │                │──> consulta analítica
bronze.ufs   ──> stg_ufs   ──┴──> dim_uf   ───┘
(Delta)                           └──> fato_taxa
```

**Repare em duas coisas contraintuitivas.**

A `fato_taxa` **não referencia a dimensão** — ela depende de `stg_taxas` e `stg_ufs`, e só. O encontro entre fato e dimensão acontece na hora da **consulta**, não na construção. Estrela é modelo lógico; DAG é ordem de construção.

E a `dim_uf` depende de `stg_ufs` — o cadastro de UF → região que dá o contexto geográfico à fato.
